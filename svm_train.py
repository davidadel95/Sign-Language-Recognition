...
def preprocess_hog(digits):
    ...
    for img in digits:
        ...
        mag, ang = cv2.cartToPolar(gx, gy) #bygeeb l magnitude wel angle bt3 el vector
        bin_n = 16
...
def hog_single(img):
	samples=[]
	gx = cv2.Sobel(img, cv2.CV_32F, 1, 0)  	#sobel x
	gy = cv2.Sobel(img, cv2.CV_32F, 0, 1)	#sobel y
