...
while(cap.isOpened()):
    ...
    img_ycrcb = cv2.cvtColor(img1, cv2.COLOR_BGR2YCR_CB) #converting the image from BGR to YCR
    blur = cv2.GaussianBlur(img_ycrcb,(11,11),0) #bluring the image
    skin_ycrcb_min = np.array((0, 138, 67))  #lower bound of skin color
    skin_ycrcb_max = np.array((255, 173, 133)) #upper bound of skin color
    mask = cv2.inRange(blur, skin_ycrcb_min, skin_ycrcb_max)  # detecting the hand in the bounding box using skin detection
    image, contours, hierarchy = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  #getting contours
    ...
    if np.any(cnt != None):
        ...
        cv2.putText(img,label,(50,150), font,8,(0,125,155),2)  # displaying the predicted letter on the screen
